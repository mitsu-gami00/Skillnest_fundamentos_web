...
// 🔢 Contador de interacciones en una app: Imagina que una aplicación registra la cantidad
// de veces que un usuario interactúa con un botón. 
// Escribe un programa que imprima los números del 1 al 10 
// para simular las interacciones de un usuario. 
...
function descuentoProgresivo() {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        let descuento = i * 5;
        alert(`Producto ${i} - Valor: ${valorProducto} - Descuento aplicado: ${descuento}% - valor final: ${valorProducto - (valorProducto * descuento / 100)}`);
    }
}

// 🎮 Calcular el total de puntos en un videojuego: Un jugador 
// gana 50 puntos cada vez que completa un nivel. 
// Escribe un programa que calcule la puntuación total 
// después de 5 niveles usando un bucle. 
// Ejemplo de salida esperada:

function calcularPuntos() {
    let puntosPorNivel = 50;
    let totalPuntos = 0;
    for (let i = 1; i <= 5; i++) {
        totalPuntos += puntosPorNivel;
        alert(`Nivel ${i} completado. Puntos acumulados: ${totalPuntos}`);
    }
...
// muestra un contador de notificaciones pendientes. 
// Escribe un programa que muestre un mensaje 
// con la cantidad de notificaciones pendientes y 
